    ...
    def _handle_PacketIn (self, event):
        ...
        def forward (message = None):
            ...
            if packet.dst.is_multicast:
                ...
                try:
                    test_portmap = (this_dpid, packet.src, packet.dst, packet.find('tcp').dstport)
                    
                    if not self.portmap.get(test_portmap):
                        test_portmap = (this_dpid, packet.src, packet.dst, packet.find('tcp').srcport)
                        
                        if not self.portmap.get(test_portmap):
                            raise AttributeError

                    new_dpid = self.portmap[test_portmap]
